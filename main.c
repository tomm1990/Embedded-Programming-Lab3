...
void main(void)
{
    ...
    while(1)							//Main is Usualy an Endless Loop
    {
		...
		if( CheckButtonPressed() ){		// If button pressed, change display
			if (flag==1) { WriteCommand(0xA7); flag = 0 ; }
			else { WriteCommand(0xA6); flag = 1; }
		}
		
		adh = ADRESH ;
		adl = ADRESL ;
		ans = ((int)adh<<8) | adl ;		// shift adh 8 bits left and make OR with adl
		//con = ans>>8;					// provides the msb of con (adh)
			
		sprintf(output, "%x", ans);		// from HEX to char[]
		WriteCommand(0x81);				// some command about screen view
		WriteCommand(ans/4);			// 1024 / 4 =~ 255
		oledPutString(output,  8/2, 9*6);	// 127 / 21 = 6 colums , 8 / 2 = 4 rows
	}
}//end main
